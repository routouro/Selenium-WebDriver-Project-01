...
public class Vip25DiscountTrial extends BaseTest {
    ...
    @Test
    public void testVip25DiscountTrial () {
        Actions.mainActions().clearSession();
        Actions.mainActions().openPage(Constants.SLEEP_SPRAYABLE_VIP_FREE_URL);

        Actions.mainActions().wait(Constants.SMALL_TIMEOUT_SECONDS);

        Actions.sleepSprayableVipFreeActions().checkPageScrolling();

        Pages.sleepSprayableVipFreePage().clickLinkBuyNow();
        Pages.sleepSprayableVipFreePage().waitReadyToTrySprayableDisappear();
