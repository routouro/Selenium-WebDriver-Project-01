...
public class SleepHacker extends BaseTest {
    ...
    @Test
    public void testSleepHacker () {
        Actions.mainActions().clearSession();
        Actions.mainActions().openPage(Constants.SLEEP_HACKER_URL);

        Actions.mainActions().wait(Constants.SMALL_TIMEOUT_SECONDS);

        Actions.sleepHackerActions().checkPageScrolling();

        Pages.sleepHackerPage().clickLinkAddToCartToCheckout();
        Pages.sleepHackerPage().waitLinkAddToCartToCheckoutDisappear();
