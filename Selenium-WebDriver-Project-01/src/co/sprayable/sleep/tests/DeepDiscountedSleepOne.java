...
public class DeepDiscountedSleepOne extends BaseTest {
    ...
    @Test
    public void testDeepDiscountedSleepOne () {
        Actions.mainActions().clearSession();
        Actions.mainActions().openPage(Constants.ENERGY_LIFETIME_BACKER_BUNDLE_SUBSCRIPTION_URL);

        Actions.mainActions().wait(Constants.MINIMUM_TIMEOUT_SECONDS);

        Actions.energyLifetimeSubscriptionActions().checkPageScrolling();

        Pages.energyLifetimeSubscriptionPage().clickLinkOrderNow();

        Pages.energyLifetimeSubscriptionPage().waitLinkOrderNowDisappear();
