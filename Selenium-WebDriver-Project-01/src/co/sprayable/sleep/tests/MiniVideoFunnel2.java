...
public class MiniVideoFunnel2 extends BaseTest {
    ...
    @Test
    public void testMiniVideoFunnel2 () {
        Actions.mainActions().clearSession();
        Actions.mainActions().openPage(Constants.SPRAYABLE_SLEEP_ULTIMATE_OFFER_URL);

        Actions.ultimateOfferActions().checkVideoPlaying();

        Actions.mainActions().openPage(Constants.SPRAYABLE_SLEEP_ULTIMATE_OFFER_URL);
        Actions.ultimateOfferActions().performAddressLineHovering();
        Pages.ultimateOfferPage().waitLinkGetMyDiscountDissapear();
        ...
        Actions.ultimateOfferActions().clickIWantMyFreeSample();
        Pages.ultimateOfferPage().waitLinkGetMyDiscountDissapear();
