...
public class Vip25DiscountV1 extends BaseTest {
    ...
    @Test
    public void testVip25DiscountV1 () {
        Actions.mainActions().clearSession();
        Actions.mainActions().openPage(Constants.SLEEP_SPRAYABLE_VIP_URL);

        Actions.mainActions().wait(Constants.SMALL_TIMEOUT_SECONDS);

        Actions.sleepSprayableVipActions().checkPageScrolling();

        Pages.sleepSprayableVipPage().clickLinkBuyNow();
        Pages.sleepSprayableVipPage().waitReadyToTrySprayableDisappear();
