...
public class DelayedShippingSleep extends BaseTest {

    public DelayedShippingSleep() {}
    ...
    @Test
    public void testDelayedShippingSleep () {
        Actions.mainActions().clearSession();
        Actions.mainActions().openPage(Constants.SUBSCRIBE_OFFER_URL);

        Actions.mainActions().wait(Constants.MINIMUM_TIMEOUT_SECONDS);

        Actions.subscriberOfferActions().checkPageScrolling();

        Pages.subscriberOfferPage().clickLinkBuyNow();

        Pages.subscriberOfferPage().waitLinkBuyNowDisappear();
